...
int main(void)
{
  ...
  while(1)
  {
	  ...
	  sprintf(accel_Buf, "Ax: %.2f Ay: %.2f Az: %.2f \r\n", Ax, Ay, Az);
	  HAL_UART_Transmit(&huart1, accel_Buf, sizeof(accel_Buf), 200);

//	  /* Gyroscope printout */
//	  uint8_t gyro_Buf[128] = { 0 };
//	  sprintf(gyro_Buf, "Gx: %.2f Gy: %.2f Gz: %.2f \r\n", Gx, Gy, Gz);
//	  HAL_UART_Transmit(&huart1, gyro_Buf, sizeof(gyro_Buf), 200);
